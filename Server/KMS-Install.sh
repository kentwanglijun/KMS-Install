...
#
# Thanks : https://github.com/zhuponfey/vlmcsd.git
#
...
install_main() {
    ...
    echo
    echo " URL: https://www.itwiki.xyz (Located in Silicon Valley, USA)"
    echo "----------------------------------------"
    echo
    ...
    if [[ x"${release}" == x"centos" ]]; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://raw.githubusercontent.com/zhuponfey/KMS-Install/master/Server/KMS; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://raw.githubusercontent.com/zhuponfey/KMS-Install/master/Server/KMS-Debian; then
            echo -e "[${red}Error:${plain}] Failed to download KMS Server script."
            exit 1
        fi
    else
    ...
    cd "${cur_dir}" || exit
    git clone https://github.com/zhuponfey/vlmcsd.git > /dev/null 2>&1
    ...
    echo "Welcome to visit: https://www.itwiki.xyz (Located in Silicon Valley, USA)"
    echo "Enjoy it!"
    echo
}
